...
erDiagram
    ...
    PLAYER ||--o{ PLAY_EVENT : "batter_id"
    PLAYER ||--o{ PLAY_EVENT : "pitcher_id"
...
> **PLAYER.bats/throws에는 `as_of_timestamp`가 없다** — 다른 모든 엔티티와
> 다른 의도적 예외다. 손잡이는 시즌 중 사실상 불변이라 시점 스냅샷이 필요
> 없다고 판단했다. 스위치히터의 그날그날 타석별 선택은 `PLAY_EVENT` 레벨
> 원자료에 있으므로 여기서 다루지 않는다.

> **`GAME_FEATURE_SNAPSHOT`은 이전 버전(9개: starter/bullpen/**closer**/lineup/
> bvp/**bench**/availability/defense/rest)과 구성이 다르다.** `closer_edge`는
> 별도 유지하지 않고 `bullpen_edge`에 통합한다(마무리도 불펜의 일부로 취급).
> `bench_edge`는 이번 버전에서 제외하고 대신 `team_matchup_edge`(상대전적,
> Bayesian shrinkage)를 새로 추가한다 — 이전 버전에서 `bench_edge`는 한 번도
> 0.0 밖의 값을 가져본 적이 없어(미구현 상태로 남아있었음) 실질적 손실이
> 없고, `team_matchup_edge`가 신호 측면에서 더 근거가 크다고 판단했다.
> 이 변경은 `features.py`/`retrosheet_etl.py`/`model.py`의 `FEATURE_NAMES`를
> 전부 재생성해야 하는 breaking change이며, 기존에 학습된 `model.json`이
> 있다면 재학습 없이는 호환되지 않는다.

> **2026-09-02 추가: `starter_edge`/`bullpen_edge`/`lineup_edge` 계산에
> 지수 감쇠(decay)를 넣었다.** 실전 라이브 분석에서 시즌 전체 평균만
> 봤다가 불펜 최근 상태와 타자 핫스트릭(최근 2경기 연속 홈런)을 놓친 사례가
> 계기다. `PitcherForm`/`BattingForm`이 이제 최근 등판·경기에 더 큰 가중치를
> 준다(반감기 근사 8~9경기). 필드명·스키마는 그대로라 breaking change
> 아니다 — 계산 방식만 바뀌었다. `lineup_edge`는 이제 0.0 고정이 아니라
> 팀 단위 감쇠가중 SLG 프록시로 실제 계산된다(선수별·좌우 스플릿은 여전히
> plays.csv 기반 v3 대상).
...
erDiagram
    ...
    CANDIDATE {
        ...
        bool is_placeholder "P4(SCORE_PROBABILITY) 완성 전이면 true - true인 동안 화면은 EV/BET 배지를 숨기고 '참고용 순위'로만 표시"
    }
    COMBINATION {
...
erDiagram
    ...
    LEG_OUTCOME {
        ...
        string event_id FK
        string settlement_version
        string outcome "WIN | LOSS | PUSH | HALF_WIN | HALF_LOSS | VOID"
    ...
    MODEL_EVALUATION {
        ...
        int sample_size
        float roi "과거 배당 스냅샷이 부족하면 null - 0이나 통과로 오인 금지"
        float roi_ci_low
        float roi_ci_high
        float max_drawdown
...
| `batting.csv` | `BATTING_APPEARANCE` | ⚠️ v1 — 팀 단위 감쇠가중 SLG 프록시로 `lineup_edge` 계산(2026-09-02 추가). 선수별·좌우 스플릿은 여전히 미구현 |
| `fielding.csv`, `teamstats.csv` | `FIELDING_APPEARANCE`, 수비 피처 | 미구현 |
