...
def parse_results(payload: dict[str, Any]) -> list[KspoMatchResult]:
    ...
    raw_items = _item_list(payload)
    required = {"match_ymd", "hteam_han_nm", "ateam_han_nm", "match_end_val"}
    for index, item in enumerate(raw_items):
        missing = sorted(key for key in required if not str(item.get(key, "")).strip())
        if missing:
            raise ValueError(
                f"unverified or incomplete data.go.kr schema at item {index}: {missing}"
            )
    return [
        KspoMatchResult(
...
def fetch_results(
    *,
    ...
    require_approved_source(
        "data_go_kr_kspo_results", automated=True, bulk=rows > 100
    )
